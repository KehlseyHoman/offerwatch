...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.dataSource.sortingDataAccessor = (app, col) => {
      switch (col) {
        case 'company':
          return (app.company + (app.roleTitle ?? '')).toLowerCase();
        case 'status':
          return app.status ?? '';
        case 'location':
          return app.location ?? '';
    ...
    this.dataSource.filterPredicate = (app, filterStr) => {
      const { text, statuses } = JSON.parse(filterStr) as {
        text: string;
  ...
  openCreate(): void {
    ...
    ref.afterClosed().subscribe((created) => {
      if (created) {
        this.load();
        this.snack.open('Application added!', '', { duration: 3000 });
      }
    });
  }
  ...
  openEdit(app: Application): void {
    ...
    ref.afterClosed().subscribe((updated) => {
      if (updated) {
        this.load();
        this.snack.open('Saved.', '', { duration: 2500 });
      }
    });
  }
  ...
  private applyFilter(): void {
    ...
    this.dataSource.filter = text || statuses.length ? JSON.stringify({ text, statuses }) : '';
  }
