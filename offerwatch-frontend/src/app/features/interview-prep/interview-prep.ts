...
export class InterviewPrepComponent implements OnInit {
  ...
  load(): void {
    this.svc.getAll().subscribe({
      next:  items => { this._items.set(items); this.loading.set(false); },
  ...
  submitAdd(): void {
    ...
    this.svc.create({ category: v.category, question: v.question, notes: v.notes || undefined })
      .subscribe({
        next: item => {
  ...
  addSuggested(category: string, question: string): void {
    ...
    setTimeout(() => document.getElementById('notes-field')?.scrollIntoView({ behavior: 'smooth' }), 80);
  }
