...
@Component({
  ...
  imports: [
    ...
    MatChipsModule, MatTooltipModule, MatDividerModule, MatExpansionModule,
  ],
...
export class BehavioralStoriesComponent implements OnInit {
  ...
  readonly filteredStories = computed(() => {
    ...
    return f === 'All'
      ? this._stories()
      : this._stories().filter(s => s.theme === f);
  });
  ...
  load(): void {
    this.svc.getAll().subscribe({
      next:  stories => { this._stories.set(stories); this.loading.set(false); },
  ...
  submitAdd(): void {
    ...
    const payload = Object.fromEntries(Object.entries(v).filter(([, val]) => val !== '' && val != null));
  ...
  themeColor(theme: string): string {
    const map: Record<string, string> = {
      Leadership: '#1565c0', Failure: '#c62828', Teamwork: '#2e7d32',
      Conflict: '#e65100', Achievement: '#6a1b9a', Growth: '#00838f',
      Communication: '#0277bd', 'Problem-Solving': '#558b2f', Innovation: '#f57f17', Other: '#555',
    };
  ...
  themeBg(theme: string): string {
    const map: Record<string, string> = {
      Leadership: '#e3f2fd', Failure: '#ffebee', Teamwork: '#e8f5e9',
      Conflict: '#fff3e0', Achievement: '#f3e5f5', Growth: '#e0f7fa',
      Communication: '#e1f5fe', 'Problem-Solving': '#f1f8e9', Innovation: '#fffde7', Other: '#f5f5f5',
    };
