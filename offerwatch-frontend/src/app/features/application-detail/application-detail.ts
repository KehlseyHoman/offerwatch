...
export class ApplicationDetailComponent implements OnInit {
  ...
  readonly primaryContact = computed(
    () =>
      this.contacts().find((c) => c.title?.toLowerCase().includes('recruit')) ??
      this.contacts()[0] ??
      null,
  );
  ...
  addNote(): void {
    ...
    this.noteSvc.create(this.appId, this.noteForm.value.body).subscribe({
      ...
      error: () => {
        this.savingNote.set(false);
        this.snack.open('Failed to save note.', '', { duration: 3000 });
      },
    });
  }
  ...
  toggleFollowup(fu: Followup): void {
    ...
    this.followups.update((list) =>
      list.map((f) => (f.id === fu.id ? { ...f, completed: next } : f)),
    );
    this.fuSvc.toggle(this.appId, fu.id, next).subscribe({
      next: (updated) =>
        this.followups.update((list) => list.map((f) => (f.id === updated.id ? updated : f))),
      error: () => {
        this.followups.update((list) =>
          list.map((f) => (f.id === fu.id ? { ...f, completed: fu.completed } : f)),
        );
